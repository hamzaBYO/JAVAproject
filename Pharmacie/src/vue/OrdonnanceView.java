...
public class OrdonnanceView extends JFrame {
    ...
    // ── En-tête ───────────────────────────────────────────────────────────────
    private JLabel lblOrdId, lblDate, lblClient;

    // ── Bloc Nouvelle Ordonnance ──────────────────────────────────────────────
    private JTextField txtOrdId, txtClientId, txtDate;
    private JButton btnCreateOrd;

    // ── Bloc Ajouter Médicament ───────────────────────────────────────────────
    private JTextField txtMedId, txtQuantite;
    private JButton btnAddLigne, btnRemoveLigne;

    // ── Table + Footer ────────────────────────────────────────────────────────
    private JTable tableDetails; private DefaultTableModel model;
    private JLabel lblTotal;
    private JButton btnClose;

    private final String ordonnanceId, dateOrd, clientInfo;
    ...
    public OrdonnanceView(String ordonnanceId, String dateOrd, String clientInfo) {
        ...
        setSize(950, 620); setLocationRelativeTo(null); setResizable(true);
    ...
    private JPanel buildHeader() {
        ...
        lblOrdId  = lbl("N° " + ordonnanceId + "   |   Date : " + dateOrd, Font.PLAIN, 12, new Color(190, 225, 255));
        lblDate   = new JLabel(); // kept for updateHeader() compatibility
        lblClient = new JLabel(); // kept for updateHeader() compatibility
    ...
    private JPanel buildCenter() {
        ...
        p.setBackground(COLOR_LIGHT); p.setBorder(new EmptyBorder(14, 18, 8, 18));
    ...
    private JPanel buildLeftPanel() {
        ...
        p.setBackground(COLOR_LIGHT); p.setPreferredSize(new Dimension(245, 0));
        p.add(buildNewOrdBlock()); p.add(vs(12)); p.add(buildAddMedBlock());
    ...
    private JPanel buildNewOrdBlock() {
        ...
        txtOrdId    = row(f, "ID Ordonnance",    "ORD001");
        txtClientId = row(f, "ID Client",        "CLT001");
        txtDate     = row(f, "Date (yyyy-MM-dd)","2025-01-01");
        f.add(vs(4));
        btnCreateOrd = actionBtn("Créer Ordonnance", COLOR_PRIMARY); f.add(btnCreateOrd);
        return f;
    }
    ...
    private JPanel buildTablePanel() {
        ...
        JPanel p = new JPanel(new BorderLayout(0, 8)); p.setBackground(COLOR_LIGHT);
        p.add(title, BorderLayout.NORTH); p.add(scroll, BorderLayout.CENTER);
        return p;
    }
    ...
    private JPanel buildFooter() {
        ...
        JPanel totalPanel = new JPanel(); totalPanel.setLayout(new BoxLayout(totalPanel, BoxLayout.Y_AXIS));
        totalPanel.setBackground(COLOR_WHITE);
        totalPanel.add(lbl("MONTANT TOTAL", Font.BOLD, 10, Color.GRAY));
        lblTotal = new JLabel("0.000 DT");
        lblTotal.setFont(new Font("Segoe UI", Font.BOLD, 28)); lblTotal.setForeground(COLOR_PRIMARY);
        ...
        btnPanel.setBackground(COLOR_WHITE); btnPanel.add(btnClose);
    ...
    private JPanel block(String title) {
        JPanel p = new JPanel(); p.setLayout(new BoxLayout(p, BoxLayout.Y_AXIS));
        p.setBackground(COLOR_WHITE); p.setAlignmentX(LEFT_ALIGNMENT);
        p.setMaximumSize(new Dimension(Integer.MAX_VALUE, Integer.MAX_VALUE));
        p.setBorder(new CompoundBorder(new LineBorder(new Color(190, 215, 250), 1, true), new EmptyBorder(12, 14, 16, 14)));
        JLabel lbl = lbl(title, Font.BOLD, 12, COLOR_PRIMARY); p.add(lbl);
        p.add(vs(6));
        JSeparator sep = new JSeparator(); sep.setForeground(new Color(210, 230, 255)); sep.setMaximumSize(new Dimension(Short.MAX_VALUE, 1));
        p.add(sep); p.add(vs(10));
        return p;
    }

    private JTextField row(JPanel form, String label, String ph) {
        form.add(lbl(label, Font.BOLD, 10, new Color(80, 110, 140))); form.add(vs(3));
        JTextField f = new JTextField(ph);
        f.setFont(new Font("Segoe UI", Font.PLAIN, 12)); f.setForeground(Color.GRAY);
        f.setMaximumSize(new Dimension(Short.MAX_VALUE, 32)); f.setAlignmentX(LEFT_ALIGNMENT);
        f.setBorder(new CompoundBorder(new LineBorder(new Color(200, 220, 245), 1, true), new EmptyBorder(4, 8, 4, 8)));
    ...
    private JTable styledTable(DefaultTableModel m) {
        ...
        t.setGridColor(new Color(210, 230, 255)); t.setSelectionBackground(COLOR_ACCENT);
        ...
        t.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            public Component getTableCellRendererComponent(JTable tbl, Object v, boolean sel, boolean foc, int row, int col) {
                ...
                if (!sel) { setBackground(row % 2 == 0 ? COLOR_WHITE : COLOR_LIGHT); setForeground(COLOR_TEXT); }
    ...
    public void addLigne(String medId, String medNom, int qty, double prix) {
        model.addRow(new Object[]{ medId, medNom, qty, String.format("%.3f", prix), String.format("%.3f", qty * prix) });
    }
    ...
    public String getNewOrdId()    { return clean(txtOrdId,    "ORD001"); }
    public String getNewClientId() { return clean(txtClientId, "CLT001"); }
    public String getNewDate()     { return clean(txtDate,     "2025-01-01"); }
    public String getMedId()       { return clean(txtMedId,    "MED001"); }
    public String getQuantiteStr() { return clean(txtQuantite, "1"); }
