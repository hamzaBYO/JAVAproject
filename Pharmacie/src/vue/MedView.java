...
public class MedView extends JFrame {
    ...
    private JTextField txtId, txtNom, txtPrix, txtStock, txtSearch;
    ...
    public MedView() { this(false); }

    public MedView(boolean readOnly) {
        this.readOnly = readOnly;
        setTitle(readOnly ? "Consulter Médicaments" : "Gestion des Médicaments");
        ...
        txtId    = field(form, "ID Médicament", "MED001");
        txtNom   = field(form, "Nom",           "Nom médicament");
        txtPrix  = field(form, "Prix (DT)",     "0.000");
        txtStock = field(form, "Stock",         "0");

        form.add(label("Type"));
        form.add(Box.createVerticalStrut(4));
        cmbType = new JComboBox<>(new String[]{"Comprimé", "Sirop", "Injection", "Pommade", "Capsule", "Autre"});
        cmbType.setMaximumSize(new Dimension(Short.MAX_VALUE, 32));
        form.add(cmbType);
        ...
        table.getTableHeader().setBackground(new Color(28, 78, 115));
        table.getTableHeader().setForeground(Color.WHITE);

        txtSearch = new JTextField();
        txtSearch.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        btnSearch = btn("Rechercher", new Color(33, 97, 140));
        btnSearch.setPreferredSize(new Dimension(120, 34));
        btnClose  = btn("✕ Fermer",   new Color(180, 60, 60));
        ...
        right.setBorder(new EmptyBorder(16, 10, 16, 16));
        right.add(bar, BorderLayout.NORTH);
        right.add(new JScrollPane(table), BorderLayout.CENTER);

        JPanel header = new JPanel(new BorderLayout());
        header.setBackground(new Color(28, 78, 115));
        header.setBorder(new EmptyBorder(12, 20, 12, 20));
        JLabel title = new JLabel(readOnly ? "Consulter Médicaments" : "Gestion des Médicaments");
    ...
    public String getMedId()    { return txtId.getText().trim(); }
    public String getMedNom()   { return txtNom.getText().trim(); }
    public String getMedPrix()  { return txtPrix.getText().trim(); }
    public String getMedStock() { return txtStock.getText().trim(); }
    public String getMedType()  { return (String) cmbType.getSelectedItem(); }
    public String getSearch()   { return txtSearch.getText().trim(); }

    public JTable            getTable() { return table; }
    public DefaultTableModel getModel() { return model; }

    /** null when isReadOnly() == true — always guarded by MedController */
    ...
    public void fillForm(String id, String nom, String prix, String stock, String type) {
        txtId.setText(id); txtNom.setText(nom); txtPrix.setText(prix);
        txtStock.setText(stock); cmbType.setSelectedItem(type);
    }

    public void clearForm() {
        txtId.setText(""); txtNom.setText(""); txtPrix.setText(""); txtStock.setText("");
    }
